...
- 如果工具栏文字可见但菜单无法通过自动化稳定展开，优先改用斜杠命令；仍不稳定时，让用户手动把光标或菜单放到目标位置后再继续。
- 修改后验证渲染结果和自动保存状态。
...
如果 `插入` 菜单没有展开，不要继续猜坐标。改用 `/表格`，或让用户手动打开 `插入` 菜单后继续选择可见的 `表格`。
...
如果 `插入` 菜单没有展开，不要继续猜坐标。改用 `/代码块`，或让用户手动打开 `插入` 菜单后继续选择可见的 `代码块`。
